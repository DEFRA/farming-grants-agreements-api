...
function validateRecord(legacyVersion) {
  ...
  try {
    ...
    issues.push(...validateSchema(candidate))
    issues.push(...validateParcels(candidate.values.parcels))
    issues.push(...checkIntegrity(legacyVersion, candidate))
  } catch (error) {
    issues.push({
      path: 'diagnostic',
...
async function checkPDFs(legacyVersion) {
  ...
  if (!legacyVersion.documents) return issues
