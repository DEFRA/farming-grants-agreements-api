...
import { unacceptAgreement } from './unaccept-agreement.js'

jest.mock('~/src/api/common/models/agreements.js')

describe('unacceptAgreement in unaccept-agreement', () => {
  ...
  test('should successfully unaccept an agreement', async () => {
    ...
    // Act
    const result = await unacceptAgreement(agreementId)
  ...
  test('should throw Boom.notFound when agreement is not found', async () => {
    ...
    // Act & Assert
    await expect(unacceptAgreement(agreementId)).rejects.toThrow(
  ...
  test('should rethrow Boom errors without wrapping', async () => {
    ...
    // Act & Assert
    await expect(unacceptAgreement(agreementId)).rejects.toEqual(boomError)
  })
})
